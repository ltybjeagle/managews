...
public class SysIdentifierService<T> {
	 ...
	 * @return
	 */
	public List<T> querySysIdentifierList(BasePage page) {
		return getDao().querySysIdentifierList(getMapperObj() + ".querySysIdentifierList", page);
	}
	
	public List<T> querySysIdentifierByUser(Integer userid) {
		return getDao().querySysIdentifierByUser(getMapperObj() + ".querySysIdentifierByUser", userid);
	}
}
