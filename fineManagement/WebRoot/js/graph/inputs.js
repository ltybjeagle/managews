$(function () {	
	...
	function onSelect(d) {
		var issd = this.id == 'sd', sd = issd ? d : new Date($('#sd').datebox('getValue')), 
				ed = issd ? new Date($('#ed').datebox('getValue')) : d;
				if (ed < sd) {
					finems.alert('警告', '结束日期小于开始日期', 'warning'); 
	...
	$('#data-list').datagrid({   
		...
		columns:[[   
		          ...
		          {field: 'baseName', title: '基地', width:100}, 
	...
	document.getElementById("ed").onfocus = function() {
		return new WdatePicker({skin: 'whyGreen', dateFmt: 'yyyy', minDate: '#F{$dp.$D(\'sd\')}'});
	}; 	
	document.getElementById("sd").onfocus = function() {
		return new WdatePicker({onpicked: function() {ed.focus();}, skin: 'whyGreen', dateFmt: 'yyyy'});
	}; 
	$("#state").combobox({
