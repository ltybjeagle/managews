var YDataGrid = function(config) {
	var istbar = typeof config.istbar == 'undefined' ? true : config.istbar;
	var btnUrl = typeof config.btnUrl == 'undefined' ? window.location.href : config.btnUrl;
	config = config || {};
	...
	var Action = {
		...
		'toAudit': actionUrl.toAudit || 'toAudit.do',
		'audit' : actionUrl.audit || 'audit.do'
	}
	// Grid DataList
	var Grid = $('#data-list');
	// Form
	...
	var Handler = {
		...
		edit: function(callback) {
			...
			if (Utils.checkSelectOne(record)) {
				...
				finems.getById(Action.getId, data, function(result) {
			...
			if (Utils.checkSelect(records)) {
				$.messager.confirm('确认', '确认删除记录?', function(r) {  
					if (r) {
				    	...
				    	var data = arr.join("&");
				    	finems.deleteForm(Action.remove, data, function(result) {
				    		finems.closeProgress();
							Events.refresh();
							// 回调函数
							if (jQuery.isFunction(callback)) {
		...
		save: function(callback) {
			if (Form.edit.form('validate')) {
				...
				finems.saveForm(Form.edit, function(data) {
					finems.closeProgress();
		...
		toAudit: function (callback) {
			var records = Utils.getCheckedRows();
			if (Utils.checkSelect(records)) {
				$.messager.confirm('确认', '送审提交后不能修改、删除，确认送审提交?', function(r) {  
					if (r) {
						finems.progress();
						var arr = [], idKey = dataGrid.idField || 'id'; // 主键名称
				    	$.each(records, function(i, record) {
				    	...
				    	var data = arr.join("&");
				    	finems.auditForm(Action.toAudit, data, function(result) {
				    		finems.closeProgress();
							Events.refresh();
							// 回调函数
							if (jQuery.isFunction(callback)) {
		...
		auditOp: function (callback) {
	    	...
	    	var data = arr.join("&");
	    	finems.auditForm(Action.audit + '?state=' + sel, data, function(result) {
	    		finems.closeProgress();
	    		Win.edit.dialog('close');
				Events.refresh();
				// 回调函数
	...
	var Utils = {
		...
		checkSelectOne: function(rows) { // 检查grid是否只勾选了一行,是返回 true,否返回true
			...
			finems.alert('警告', '只能选择一行记录.', 'warning');  
			return false;
		}
	}
	// 自定义事件
	...
	var Events = {
		...
		toAudit : evt.toAudit || Handler.toAudit,
		audit : evt.audit || Handler.audit
	}
	...
	var bar_toaudit = {
			id: 'btntoaudit',
			text: '送审',
			iconCls: 'icon-to_audit',
			btnType: 'toaudit',
			handler: Events.toAudit
	};
	var bar_audit = {
			id: 'btnaudit',
			text: '审核',
	...
	var getToolbar = function () {
		...
		if (dataGrid.toolbar != undefined && dataGrid.toolbar.length > 0) {
			for (var i = 0; i < dataGrid.toolbar.length; i++) {
				...
				if (bar.btnType == 'add') {
					tbars.push({id: bar.id || bar_add.id, text: bar.text || bar_add.text, 
						iconCls: bar.iconCls || bar_add.iconCls, btnType: bar.btnType || bar_add.btnType, 
						handler: bar.handler || bar_add.handler});
					continue;
				}
				if (bar.btnType == 'edit') {
					tbars.push({id: bar.id || bar_edit.id, text: bar.text || bar_edit.text, 
						iconCls: bar.iconCls || bar_edit.iconCls, btnType: bar.btnType || bar_edit.btnType, 
						handler: bar.handler || bar_edit.handler});
					continue;
				}
				if (bar.btnType == 'remove') {
					tbars.push({id: bar.id || bar_remove.id, text: bar.text || bar_remove.text, 
						iconCls: bar.iconCls || bar_remove.iconCls, btnType: bar.btnType || bar_remove.btnType, 
						handler: bar.handler || bar_remove.handler});
					continue;
				}
				if (bar.btnType == 'toaudit') {
					tbars.push({id: bar.id || bar_toaudit.id, text: bar.text || bar_toaudit.text, 
						iconCls: bar.iconCls || bar_toaudit.iconCls, btnType: bar.btnType || bar_toaudit.btnType, 
						handler: bar.handler || bar_toaudit.handler});
					continue;
				}
				if (bar.btnType == 'audit') {
					tbars.push({id: bar.id || bar_audit.id, text: bar.text || bar_audit.text, 
						iconCls: bar.iconCls || bar_audit.iconCls, btnType: bar.btnType || bar_audit.btnType, 
						handler: bar.handler || bar_audit.handler});
					continue;
				}
				tbars.push({id: bar.id, text: bar.text, iconCls: bar.iconCls, btnType: bar.btnType, handler: bar.handler, 
