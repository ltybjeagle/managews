...
var finems = {
	...
	isJson: function(str) {
		var obj = null; 
		try {
			obj = finems.paserJson(str);
		} catch(e) {
			return false;
		}
		var result = typeof(obj) == "object" && Object.prototype.toString.call(obj).toLowerCase() == "[object object]" 
			&& !obj.length; 
		return result;
	},
	...
	/*重新登录页面*/
	toLogin: function() {
		window.top.location = urls['msUrl'] + "/sysMain/login.shtml";
	},
	checkLogin: function(data) { // 检查是否登录超时
		if (data && data.logoutFlag) {
			finems.closeProgress();
			finems.alert('提示', "登录超时,点击确定重新登录.", 'error', finems.toLogin);
			return false;
		}
		return true;
	},
		 	...
		 	error: function(response, textStatus, errorThrown) {
		 		try {
			 		...
			 		if (!finems.checkLogin(data)) {
			 			...
			 			finems.alert('提示', data.msg || "请求出现异常,请联系管理员", 'error');
				 	}
		 		} catch(e) {
		 			finems.alert('提示', "请求出现异常,请联系管理员.", 'error');
		 		}
		 	},
		 	complete:function(){
		 	}
	...
	submitForm: function(form, callback, dataType) {
		...
		finems.ajaxSubmit(form, option);
	},
	saveForm: function(form, callback) {
		if (form.form('validate')) {
			finems.progress('Please waiting', 'Save ing...');
			//	ajax提交form
			finems.submitForm(form, function(data) {
				finems.closeProgress();
				if (data.success) {
					if (callback) {
						callback(data);
					} else {
						finems.alert('提示', '保存成功.', 'info');
					} 
				} else {
					finems.alert('提示', data.msg, 'error');  
				}
			});
		}
	},
	...
	getById: function(url, option, callback) {
		finems.progress();
		finems.ajaxJson(url, option, function(data) {
			finems.closeProgress();
			if (data.success) {
				if (callback) {
					callback(data);
